...
Single-page app, **vanilla TypeScript + Vite**, near-zero runtime dependencies. MIT-licensed (making the verifier open is the point). It is one of the ar.io verification stack siblings and depends on [`ar-io-agent`](../ar-io-agent) **for specification only — no code dependency** (envelope schema, the `Asset-Hash` tag convention, and the conformance test vectors).
...
| `hash.ts` | In-browser SHA-256 of the dropped file (one-shot; streaming WASM for huge files is a future hardening step). |
| `gateway.ts` | The only outbound traffic. `findEnvelopeTxs` unions GraphQL queries over the three content-hash tags (`Asset-Hash`/`Observed-Hash`/`Baseline-Hash`) and dedupes. `findAssetEventTxs` queries all of an asset's events by `Asset-Id` (+ block timestamps). `fetchEnvelope` does `/raw/<tx>`. Default gateway `turbo-gateway.com`; multi-gateway fallback is a future step. |
...
- **GraphQL lag:** freshly-anchored txs may not be tag-indexed for seconds–minutes; `/raw` is immediate. "no-match" can mean "not indexed yet."
